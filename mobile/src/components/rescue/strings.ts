...
/**
 * Copy for the overheat rescue card.
 *
 * Kept out of the shared catalogues so this feature can settle without the
 * three locale files moving in step. Typing `zh` and `ko` as `Copy` makes a
 * missing translation a compile error rather than a blank label on a phone.
 *
...
const en = {
  ...
  titleBoth: 'Cooking and starved',
  losingLabel: 'Bleeding right now',
  losingUnit: 'VOLTS / h',
  stabilityLabel: 'Stability',
  diagnosisHeat: 'Cooling covers only {n}% of the heat your parts make, so every core is throttled to match.',
  diagnosisPower: 'Supply covers only {n}% of the watts your parts draw, and a brownout cuts harder than heat.',
  diagnosisBoth: 'Cooling covers {cool}% of the heat, supply covers {pow}% of the draw. The penalties multiply.',
  fixLabel: 'The fix',
...
/** Same keys as `en`, with plain `string` values so a translation may differ. */
...
const zh: Copy = {
  ...
  diagnosisPower: '供电只覆盖了部件功耗的 {n}%，掉电比过热更伤产出。',
...
const ko: Copy = {
  ...
  titleBoth: '과열에 전력 부족까지',
  losingLabel: '지금 새어나가는 양',
  losingUnit: 'VOLTS / h',
  stabilityLabel: '안정도',
  diagnosisHeat: '냉각이 부품 발열의 {n}%만 감당하고 있어, 모든 코어가 그만큼 제한됩니다.',
  diagnosisPower: '공급이 부품 소비 전력의 {n}%만 감당합니다. 전력 부족은 발열보다 더 크게 깎습니다.',
  diagnosisBoth: '냉각은 발열의 {cool}%, 공급은 소비 전력의 {pow}%를 감당합니다. 페널티는 곱해집니다.',
  fixLabel: '해결책',
...
/** Substitutes `{name}` placeholders, matching the catalogue's interpolation. */
